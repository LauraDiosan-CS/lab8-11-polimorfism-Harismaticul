...
//Out: -
void ServiceA::addElem(int id,string nume,string email,int grad)
{
...
//Out: -
void ServiceA::updateElem(int id1,int id2,string nume,string email,int grad)
{
...
bool ServiceA::checkUser(string User,string Pass)
{
    ...
    for(i=0;i<r.getSize();i++)
    {
        string id;
        string password;
        id=to_string(elements[i].getid());
        password=elements[i].getnume()+id;
